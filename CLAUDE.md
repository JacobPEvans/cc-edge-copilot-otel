...
Cribl Edge pack for GitHub Copilot Chat OpenTelemetry collection.
...
5. Create GitHub release with the `.crbl` artifact built locally:
   ```sh
   tar -czf cc-edge-copilot-otel-vX.Y.Z.crbl data default package.json README.md
   cp cc-edge-copilot-otel-vX.Y.Z.crbl cc-edge-copilot-otel.crbl
   gh release create vX.Y.Z --draft \
     --repo JacobPEvans/cc-edge-copilot-otel \
     cc-edge-copilot-otel-vX.Y.Z.crbl cc-edge-copilot-otel.crbl
   gh release edit vX.Y.Z --repo JacobPEvans/cc-edge-copilot-otel --draft=false
   ```
